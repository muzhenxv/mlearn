...
class AppCateEncoder(BaseEstimator, TransformerMixin):
    def __init__(self, cate_dict, delimiter='|', prefix='app_', unknown='unknown'):
        ...
        if type(cate_dict) == str:
            ...
            # v2版本app分类资料文件需要做这个处理，因为文件本身是个dataframe
            cate_dict = cate_dict.drop_duplicates('name')
            cate_dict = dict(zip(cate_dict['name'], cate_dict['categories'].str[4:]))
    ...
    def transform(self, df):
        dfs = df.squeeze(axis=1).map(lambda s: self.app2cate(s, self.cate_dict, self.delimiter, self.prefix, self.unknown))
...
if __name__ == '__main__':
    ...
    root_path = os.path.abspath(__file__).rsplit('/mlearn', 1)
    cate_dict = os.path.join(root_path, 'mlearn/materials/cate_dict_20181204_v2.pkl')
    enc = AppCateEncoder(cate_dict=cate_dict)
